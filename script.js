...
const el_scene_calculating = document.getElementById("game-calculating");
...
async function hint(grid){
	...
	if(wasmGetHint) hint = wasmGetHint(grid.toString());
	// Otherwise use the plain JS version
	else hint = Solver.getHint(grid.toString());
	console.log(hint);
	return hint;
}
...
game.addScene("Level", new Scene(function(gotoScene, data){
	...
	ui: {
		buttons: [
			...
			new UIButton(btn_game_hint, async function(gotoScene, data){
				let g = data.gameBoard;
				if(g.isAnimating) return;
				blur(el_scene_game);
				show(el_scene_calculating);
				getComputedStyle(el_scene_calculating);
				applyMove(g, await hint(g));
				hide(el_scene_calculating);
